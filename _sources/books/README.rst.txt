========================================
書籍紀錄
========================================


書籍筆記以 ISBN 為開頭、書名接在後頭作為檔案名稱，
例如「 ``ISBN-13|978-1-59327-385-9|A Bug Hunter's Diary.rst`` 」。


.. code-block:: python

    "ISBN-{}|{}|{}.rst".format(13, "978-1-59327-385-9", "A Bug Hunter's Diary")


已讀完
========================================

有很多以前讀過的沒有紀錄在這邊，
哪天意外想到的時候再加進來。

* Grokking Algorithms: An illustrated guide for programmers and other curious people
* 植物獵人的茶盜之旅：改變中英帝國財富版圖的茶葉貿易史（For All the Tea in China: How England Stole the World's Favorite Drink and Changed History）
* C++ API 設計（API Design for C++）
* 設計的設計：一位電腦科學家的設計歷險（The Design of Design: Essays from a Computer Scientist）
* 黑客列傳：電腦革命俠客誌
* 約耳續談軟體：探究軟體經營的根本實學
* 約耳趣談軟體：來自專案管理的現場實錄
* 老人與海（The Old Man and the Sea）
    - 小時候讀的
* 動物農莊（Animal Farm）
    - 小時候讀的
* 2001 太空漫遊（2001: a Space Odyssey）
    - 看過電影
* 在天堂遇見的五個人
    - 小時候讀的
* 追風箏的孩子
    - 小時候讀的


已購買
========================================

（2017/02/27 於讀冊上購買的二手書）：

* 程式設計師的自我修養：連結、載入、程式庫
* 人月神話：軟體專案管理之道
* 昨日世界：找回文明新命脈
* 費瑪最後定理
* 大崩壞
* 獨裁者的進化：收編、分化、假民主
* 這不是英語：從語言看英美文化差異的第一手觀察誌


（2017/02/24 於 Kobo 上購買的電子書）：

* 細菌：我們的生命共同體
* 人類的故事（房龍經典名著，21 世紀 90 章最新增訂）
* 開膛史
* 海洋帝國：決定伊斯蘭與基督教勢力邊界的爭霸時代


（2017/01/11 在博客來上面搭著年前特惠購買之實體書）：

* 碼書：編碼與解碼的戰爭
* 掛在地圖上的狂想者：幾條現、幾種色、幾個點，一個既大又小、既扁又圓的祕密世界
* 血之祕史：科學革命時代的醫學與謀殺故事
* 植物獵人的茶盜之旅：改變中英帝國財富版圖的茶葉貿易史
* 醉人植物博覽會
* 智慧宮：被掩蓋的阿拉伯知識史


（2016/11/26 陸軍週休在 Humble Bundle 上購買之電子書）：

* Bash Pocket Reference, 2nd Edition
* Classic Shell Scripting
* DNS and BIND, 5th Edition
* Essential System Administration, 3rd Edition
* Learning GNU Emacs, 3rd Edition
* Learning Unix for OS X, 2nd Edition
* Learning the bash Shell, 3rd Edition
* Learning the vi and Vim Editors, 7th Edition
* Linux Pocket Guide, 3rd Edition
* Network Troubleshooting Tools
* TCP/IP Network Administration, 3rd Edition
* Ten Steps to Linux Survival
* Unix Power Tools
* Unix in a Nutshell, 4th Edition
* bash Cookbook
* lex and yacc, 2nd Edition
* sed & awk, 2nd Edition


（2016/10/23 陸軍週休於「博客來 2016 碁峰暢銷展 2 書以上 75 折」購買的實體書）：

* The Linux Programming Interface 國際中文版（上冊）（下冊）
    - 作者：Michael Kerrisk
    - Linux man-pages 的維護者
    - `書籍勘誤表 <http://tlpi.netdpi.net/>`_
* C++ 並行程式設計實戰手冊（C++ Concurrency in Action）
    - 作者：Anthony Williams
    - C++11 std::thread 的作者
* C++ API 設計（API Design for C++）
    - 作者：Martin Reddy
* 設計的設計：一位電腦科學家的設計歷險（The Design of Design: Essays from a Computer Scientist）
    - 作者：Frederick P. Brooks
    - 作者其他書籍
        + 沒有銀彈
        + 人月神話
        + 再論人月神話


（於 2016/08/27 在 Humble Bundle 上購買之電子書）：

* Art of R Programming: A Tour of Statistical Software Design
* Automate the Boring Stuff with Python: Practical Programming for Total Beginners
* Clojure for the Brave and True: Learn the Ultimate Language and Become a Better Programmer
* Eloquent JavaScript, 2nd edition: A Modern Introduction to Programming
* If Hemingway Wrote JavaScript
* Land of Lisp: Learn to Program in Lisp, One Game at a Time!
* Learn You A Haskell for Great Good!
* Learn You Some Erlang for Great Good!
* Python Playground: Geeky Projects for the Curious Programmer
* Realm of Racket: Learn to Program, One Game at a Time!
* The Book of F#: Breaking Free with Managed Functional Programming
* The Linux Command Line: A Complete Introduction
* The Principles of Object-Oriented JavaScript
* Think Like a Programmer: An Introduction to Creative Problem Solving
* Write Great Code, Volume 1: Understanding the Machine
* Write Great Code, Volume 2: Thinking Low-Level, Writing High-Level


（於 2016/05/07 在 Humble Bundle 上購買之電子書）：

* A Bug Hunter's Diary: A Guided Tour Through the Wilds of Software Security
* Automate the Boring Stuff with Python
* Bitcoin for the Befuddled
* Black Hat Python: Python Programming for Hackers and Pentesters
* Designing BSD Rootkits: An Introduction to Kernel Hacking
* Hacker Bundle 2016 PDF Sampler
* Hacking the Xbox: An Introduction to Reverse Engineering
* Hacking: The Art of Exploitation, 2nd Edition
* Practical Malware Analysis: The Hands-On Guide to Dissecting Malicious Software
* Python Crash Course: A Hands-On, Project-Based Introduction to Programming
* Silence on the Wire: A Field Guide to Passive Reconnaissance and Indirect Attacks
* Smart Girl's Guide to Privacy: Practical Tips for Staying Safe Online
* The Linux Command Line
* The Maker's Guide to the Zombie Apocalypse: Defend Your Base with Simple Circuits, Arduino, and Raspberry Pi



英文 DRM-Free 電子書
========================================

* `O'Reilly Media <http://shop.oreilly.com/>`_
* `The Pragmatic Bookshelf <https://pragprog.com/>`_
* `Manning <https://www.manning.com/>`_
* `InformIT <http://www.informit.com/>`_



書籍推薦來源
========================================

* `Top mentioned books on stackoverflow.com <http://www.dev-books.com/>`_
* `Hacker News Books <http://hackernewsbooks.com/>`_
* PTT Soft_Job 賞書系列
